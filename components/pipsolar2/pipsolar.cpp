  ...
  if (this->state_ == STATE_POLL_DECODED) {
    ...
    switch (this->used_polling_commands_[this->last_polling_command_].identifier) {
      ...
      case POLLING_QPIWS:
        ...
        break;
	  case POLLING_HGEN:
        if (this->daily_electricity_generation_) {
          this->daily_electricity_generation_->publish_state(value_daily_electricity_generation_);
        }
		if (this->monthly_electricity_generation_) {
          this->monthly_electricity_generation_->publish_state(value_monthly_electricity_generation_);
        }
		if (this->annual_electricity_generation_) {
          this->annual_electricity_generation_->publish_state(value_annual_electricity_generation_);
        }
		if (this->total_electricity_generation_) {
          this->total_electricity_generation_->publish_state(value_total_electricity_generation_);
        }
	  case POLLING_HPVB:
  ...
  if (this->state_ == STATE_POLL_CHECKED) {
    ...
    switch (this->used_polling_commands_[this->last_polling_command_].identifier) {
	  ...
	  case POLLING_HGEN:
        ...
        sscanf(                                                                                              // NOLINT
            tmp,                                                                                             // NOLINT
            "(%d %d:%d %f %f %f %f",  // NOLINT
            &value_today_date_, &value_now_time_hour_, &value_now_time_min_,                                 // NOLINT
            &value_daily_electricity_generation_,                                                                  // NOLINT
            &value_monthly_electricity_generation_, &value_annual_electricity_generation_,                   // NOLINT
			&value_total_electricity_generation_);                                                          // NOLINT
...
uint8_t Pipsolar::check_incoming_crc_() {
  uint16_t crc16;
  uint8_t hgen[] = {'H', 'G', 'E', 'N', '\0'};
  uint8_t hpvb[] = {'H', 'P', 'V', 'B', '\0'};
  ...
  else if (memcmp(this->used_polling_commands_[this->last_polling_command_].command, hgen, sizeof(hgen)) == 0){
	  ESP_LOGD(TAG, "This does not require CRC check.");
	  return 1;
  }
  else if (memcmp(this->used_polling_commands_[this->last_polling_command_].command, hpvb, sizeof(hpvb)) == 0){
	  ESP_LOGD(TAG, "This does not require CRC check.");
	  return 1;
  }
...
void Pipsolar::send_next_poll_() {
 ...
 // ESP_LOGD(TAG, "NOW: %s", this->last_polling_command_);
  ...
  this->write_array(this->used_polling_commands_[this->last_polling_command_].command,
                    this->used_polling_commands_[this->last_polling_command_].length);
  // checksum
  if (memcmp(this->used_polling_commands_[this->last_polling_command_].command, hgen, sizeof(hgen)) == 0){
  }
  else if (memcmp(this->used_polling_commands_[this->last_polling_command_].command, hpvb, sizeof(hpvb)) == 0){
  }
  else {
  ...
  // end Byte
  this->write(0x0D);
  ESP_LOGD(TAG, "Sending polling command : %s with length %d and %d",
